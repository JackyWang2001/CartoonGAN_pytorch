...
def edge_promoting(root, save):
    """
    Create dataset for anime images without clear edges
    :param root: the source image folder
    :param save: the target image folder (to store the blurred images)
    """
    file_list = glob.glob(os.path.join(root, '*'))
